...
export class GameComponent implements OnInit {
    ...
    this.socket.on('postMoves', (data) => {
      ...
      for (let index = 0; index < this.moveList.length; index++) {
        ...
        if (block.includes('O-')) {
          ...
          // King Side
          if (block === 'O-O') {
            if (this.colourTurn === 'w') block = 'g1'; // king > g1   rook > f1
            if (this.colourTurn === 'b') block = 'g8'; // king > g8   rook > f8
          }

          // Queen Side
          if (block === 'O-O-O') {
            if (this.colourTurn === 'w') block = 'c1'; // king to c1   rook > d1
            if (this.colourTurn === 'b') block = 'c8'; // king > c8    rook > d8
          }
        }
        if (block.includes('=')) block = block.replace(/(?<==)[^\]]+/, ''); // PROMOTION // replace everything after = // can refactor
      ...
      if (checkMove) {
        ...
        if (checkMove.flags.includes('e')) {
          ...
          if (checkMove.color === 'w')
            passantBlockID =
              this.block.id[0] + (parseInt(this.block.id[1]) - 1).toString();
          else
            passantBlockID =
              this.block.id[0] + (parseInt(this.block.id[1]) + 1).toString();

          this.removePieceFromChildNode(
            document.getElementById(passantBlockID)
          );
        }
        ...
        if (checkMove.flags.includes('p')) {
          ...
          this.addPieceToChildNode(
            document.getElementById(checkMove.to),
            checkMove.promotion,
            '',
            checkMove.color
          );
  ...
  mod(a: number) {
    if (a % 8 == 0) {
      // invert colour
      this.blockColour1 =
        this.blockColour1 == 'light-section' ? 'dark-section' : 'light-section';
      this.blockColour2 =
        this.blockColour2 == 'dark-section' ? 'light-section' : 'dark-section';
    }
  ...
  drag(ev) {
    ...
    if (ev.target.id[0] == 'w' && colour !== 'white') return;
    if (ev.target.id[0] == 'b' && colour !== 'black') return;
  ...
  drop(ev) {
    ...
    this.currentPiece = this.fetchPieceFromChildNode(
      document.getElementById(this.pieceLastPosition)
    );

    if (
      this.currentPiece.id.includes('pawn') &&
      (this.block.id.includes('8') || this.block.id.includes('1'))
    ) {
      this.showPromotion = true; // also how to subscribe unil the user is done selecting a piece //receiveSelectedPromotion

      this.promotionSelectionService.promotionSelection.subscribe(
        (pieceData: string) => {
  ...
  fetchPieceFromChildNode(blockNode) {
    ...
    blockNode.firstChild.childNodes.forEach((element) => {
      ...
      if (name === 'div') piece = element;
    });
    return piece;
  }

  swapPieceFromChildNode(oldBlockNode, newBlockNode) {
    newBlockNode.firstChild.appendChild(
      this.fetchPieceFromChildNode(oldBlockNode)
    );
  }
      ...
      case 'r':
        blockNode.firstElementChild.firstElementChild.innerHTML = `
          ...
          <g inkscape:label="Layer 1" inkscape:groupmode="layer" id="layer1"
            ...
            <path fill="${
              pieceColour === 'b' ? this.pieceDark : this.pieceLight
            }"
